...
import { ShieldCheck, ChevronRight, Check, ChevronLeft } from 'lucide-react';
...
const SeniorPlanDetailPage: React.FC = () => {
  ...
  const { category: categoryRouteParam, variant: variantRouteParam } = useParams();
  const variantParam = ((variantRouteParam || searchParams.get('variant') || 'single') as string).toLowerCase();
  const categoryParam = (categoryRouteParam || searchParams.get('category') || '').toString();
  const [option, setOption] = useState('');
  const [quantity, setQuantity] = useState(1);
  // Related products state (match Day-to-Day layout) – Senior only has Single & Couple
  type CardKey = 'single' | 'couple';
  const [expanded, setExpanded] = useState<Record<CardKey, boolean>>({ single: false, couple: false });
  const toggleExpanded = (key: CardKey) =>
    setExpanded((prev) => ({ single: false, couple: false, [key]: !prev[key] }));
  // Senior has 3 categories: Day-to-Day, Comprehensive, Hospital
  const normalizeCategory = (raw: string) => {
    const v = (raw || '').toLowerCase();
    if (v.includes('day')) return 'Day-to-Day';
    if (v.includes('comp')) return 'Comprehensive';
    if (v.includes('hosp')) return 'Hospital';
    return 'Day-to-Day';
  };
  const displayCategory = normalizeCategory(categoryParam);
  const pageTitle = `Senior - ${displayCategory}`;

  // Match Day-to-Day layout (cover chips, tabs, paginated description)
  const coverItems = [
  ...
  const descriptionItemsDayToDay: { title: string; text: string }[] = [
    {
...
Day1 Health complies with the principles of open enrollment, community rating and cross-subsidisation and does not discriminate or refuse membership on the basis of race, age, gender, marital status, ethnic or social origin, sexual orientation, pregnancy, disability, state of health, geographical location or any other means of discrimination.`;
  ...
  const pageCount = Math.ceil((displayCategory === 'Day-to-Day' ? descriptionItemsDayToDay.length : 0) / pageSize) || 1;
  const pagedItems = (displayCategory === 'Day-to-Day' ? descriptionItemsDayToDay : []).slice(
    page * pageSize,
  ...
  const pricing = {
    ...
    // Value Plus Senior Hospital Plan pricing from docs
                    ...
                    >
                      {displayCategory === 'Day-to-Day' ? (
                            ...
                            <div className="mt-6 flex items-center justify-center gap-3">
                              <button
                                ...
                                className={`inline-flex items-center justify-center h-9 w-9 rounded-full border transition ${
                                  isDark
                                    ? 'border-gray-700 text-gray-200 hover:bg-gray-700/60 disabled:opacity-40'
                        ...
                        <div className={`text-sm ${isDark ? 'text-gray-200' : 'text-gray-800'}`}>
                          Detailed benefits coming soon for this Senior category.
                        </div>
                      )}
                    </motion.div>
                  ) : (
                    ...
                    >
                      ...
                      <div>
                        ...
                        <ul className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2">
                          {['Single Member', 'Couple'].map((opt) => (
                            <li key={opt} className={`rounded-lg border px-3 py-2 text-sm ${isDark ? 'bg-gray-900/60 border-gray-700 text-gray-200' : 'bg-white border-gray-200 text-gray-800'}`}>{opt}</li>
                          ))}
                        </ul>
                      </div>
                    </motion.div>
            ...
            {displayCategory === 'Day-to-Day' && (
              <section className="mt-8">
                <div className={`max-w-[74rem] mx-auto px-4 md:px-6`}>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 items-start justify-items-center">
                    {/* Single */}
                    <motion.div
                      className={`relative self-start group rounded-2xl shadow-lg p-5 border-2 transition-all overflow-visible transform-gpu max-w-[420px] w-full ${
                        isDark ? 'bg-gray-800 border-green-700 hover:border-green-500' : 'bg-white border-green-200 hover:border-green-400'
                      } min-h-[140px]`}
                    ...
                    >
                      {expanded.single && (
                        ...
                        >
                          <img src="/assets/images/daytoday/single/daytodaysingle.png" alt="" className="w-full h-full object-cover" loading="lazy" />
                      ...
                      <div className="mb-[17px]">
                        <AnimatePresence mode="wait" initial={false}>
                          {expanded.single ? (
                            <motion.div
                              key="hdr-expanded-senior-single"
                            ...
                            >
                              <motion.span
                                className={`inline-flex items-center rounded-md px-2 py-0.5 border backdrop-blur-sm ${
                                  isDark ? 'bg-emerald-500/10 border-emerald-200/20' : 'bg-emerald-500/10 border-emerald-500/20'
                            ...
                            <motion.h3
                              key="hdr-collapsed-senior-single"
                      ...
                      {expanded.single && (
                        <motion.div
                          className={`relative z-10 mb-4 inline-flex items-baseline gap-2 rounded-xl border backdrop-blur-sm px-3 py-1 ${
                            isDark ? 'bg-emerald-500/10 border-emerald-200/20' : 'bg-emerald-500/10 border-emerald-500/20'
                          }`}
                      ...
                      <div className={(expanded.single ? 'mt-[-3px] ' : 'mt-6 ') + 'relative z-10'}>
                        <AnimatedPaymentButton
                          text="Choose Plan"
                          className="bronze"
                          hoverMessages={['GP and specialist consultations','Acute and chronic medication','Blood tests and x-rays','Dentistry and optometry']}
                        ...
                        <button
                          ...
                          className={`absolute left-1/2 -translate-x-1/2 bottom-[-36px] inline-flex items-center justify-center w-8 h-8 rounded-full border backdrop-blur-sm z-[999]
                            ...
                            ${isDark ? 'bg-gray-900/60 border-white/15 text-white ring-1 ring-white/10' : 'bg-white/80 border-gray-200 text-gray-800 ring-1 ring-black/5'}
                            ${expanded.single ? 'rotate-180' : ''}`}
                          onClick={() => toggleExpanded('single')}
                        >
                          <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" aria-hidden="true"><path d="M7.41 8.59L12 13.17l4.59-4.58L18 10l-6 6-6-6z" /></svg>
                        </button>
                      </div>
                    </motion.div>
                    ...
                    >
                      ...
                      <div className="mb-[17px]">
                        <AnimatePresence mode="wait" initial={false}>
                          {expanded.couple ? (
                            <motion.div
                              key="hdr-expanded-senior-couple"
                              className={`relative z-20 flex items-center justify-between`}
                              initial={{ opacity: 0, y: -6 }}
                              animate={{ opacity: 1, y: 0 }}
                              exit={{ opacity: 0, y: 6 }}
                            ...
                            <motion.h3
                              key="hdr-collapsed-senior-couple"
                              className={`text-xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}
                              initial={{ opacity: 0, y: -6 }}
                              animate={{ opacity: 1, y: 0 }}
                              exit={{ opacity: 0, y: 6 }}
                      ...
                      {expanded.couple && (
                        <motion.div
                          ...
                          transition={{ type: 'tween', duration: 0.22, ease: [0.4, 0.0, 0.2, 1] }}
                        >
                          <span className="text-2xl font-bold text-emerald-400">R850</span>
                          <span className={`text-white text-sm font-normal`}>/month</span>
                        </motion.div>
                      )}
                      <motion.div
                      ...
                      <div className={(expanded.couple ? 'mt-[-3px] ' : 'mt-6 ') + 'relative z-10'}>
                        <AnimatedPaymentButton
                          ...
                          to="/senior/day-to-day/couple?category=Day-to-Day&variant=couple&qty=1"
                        />
                        <button
                          type="button"
